...
def _enviar_email(destinatarios, titulo, corpo, arquivos):
    ...
    if not email_usuario or not chave:
        st.error("Por favor, configure seu e‑mail e sua chave na página de Configuração.")
        return
...
def home():
    ...
    col1, _, col3 = st.columns([0.4, 0.2, 0.4])
    col1.button(
        "Enviar email",
...
def pag_configuracao():
    ...
    if st.button("Salvar email", key="salvar_email"):
        _salvar_email(email)
...
def main():
    ...
    if pagina == "home":
        ...
        txt  = st.session_state.get("texto_template_editar", "")
        pag_adicionar_novo_template(nome, txt)
    elif pagina == "lista_emails":
        pag_lista_email()
    elif pagina == "adicionar_nova_lista":
        pag_adicionar_nova_lista()
    elif pagina == "editar_lista":
        nome = st.session_state.get("nome_lista_editar", "")
        txt  = st.session_state.get("texto_lista_editar", "")
        pag_adicionar_nova_lista(nome, txt)
